...
int ft_check_strtab_and_symtab_32(int fd, t_elf32_ehdr elf32, size_t file_size)
{
	int		i;
	int     	rd;
	int		strtab_exist;
	...
	sections = (t_elf32_shdr *) malloc(elf32.e_shentsize * elf32.e_shnum);
	strtab_exist = 0;
	symtab_exist = 0;
	...
	{
		rd = read(fd, &sections[i], elf32.e_shentsize);
		if (sections[i].sh_type == 2 || sections[i].sh_type == 3)
		{
			printf("index, readed bytes : %d %d\n", i, rd);
                         printf("name      : %d\n",   sections[i].sh_name);
	...
	close(fd);
	ft_loop_over_symbols_32(symtab, strtab, sections, "read_elf_header.o");
	return (0);
}
